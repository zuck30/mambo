...
CREATE TABLE cars (
  ...
  year INTEGER,
  color TEXT,
  car_type TEXT, -- sedan, SUV, truck, etc.
  engine_size TEXT,
  image_url TEXT,
  notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE TABLE services (
  ...
  category TEXT, -- 'wash', 'detailing', 'engine', 'tire', etc.
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE jobs (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  job_number TEXT UNIQUE NOT NULL, -- auto-generated e.g. KAZI-20240101-001
...
CREATE TABLE expenses (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  branch_id UUID REFERENCES branches(id),
  category TEXT NOT NULL, -- 'supplies', 'utilities', 'salary', 'maintenance', etc.
...
CREATE TABLE inventory_items (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT NOT NULL,
  unit TEXT NOT NULL, -- 'liters', 'pieces', 'kg', etc.
...
-- CRUD policies (simplified for the prompt)
...
-- Policy: only admin/manager can delete
...
BEGIN
  RETURN 'RISITI-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' ||
    LPAD((EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT::TEXT, 6, '0');
END;
$$ LANGUAGE plpgsql;
...
-- Enable Realtime on jobs table (for live queue)
