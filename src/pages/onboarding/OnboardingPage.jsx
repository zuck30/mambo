...
const OnboardingPage = () => {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      navigate('/app/home');
      toast.success('Welcome to mambo!');
    } catch (error) {
  ...
  const steps = [
    {
      ...
      content: (
        <input
          ...
          className="w-full bg-transparent border-b-2 border-white/20 py-4 text-3xl text-white focus:outline-none focus:border-theme-yellow transition-colors"
          autoFocus
        />
      ),
          ...
          {['male', 'female'].map(g => (
            <button
              ...
              className={`w-full py-4 rounded-full border-2 transition-all text-xl capitalize ${
                formData.gender === g ? 'border-theme-yellow bg-theme-yellow/10 text-white' : 'border-white/20 text-dark-text'
              }`}
            >
              {g}
            </button>
        ...
        <div className="space-y-4">
          {['men', 'women', 'everyone'].map(pref => (
            <button
              ...
              className={`w-full py-4 rounded-full border-2 transition-all text-xl capitalize ${
                formData.show_gender === pref ? 'border-theme-yellow bg-theme-yellow/10 text-white' : 'border-white/20 text-dark-text'
              }`}
            >
              {pref}
            </button>
            ...
            <button
              ...
              className={`w-full p-4 rounded-2xl border-2 text-left transition-all flex items-center gap-4 ${
                formData.relationship_goal === goal.label
                  ? 'border-theme-yellow bg-theme-yellow/10 text-white'
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col items-center justify-center space-y-6">
          <div className="w-24 h-24 rounded-full bg-theme-yellow/10 flex items-center justify-center">
            <motion.div
              animate={{ scale: [1, 1.2, 1] }}
              transition={{ repeat: Infinity, duration: 2 }}
            >
              <Check size={48} className="text-theme-yellow" />
            </motion.div>
          </div>
          <button
            onClick={() => {
      ...
      <div className="max-w-md mx-auto w-full flex-grow flex flex-col">
          ...
          {steps.map(s => (
            <div
              ...
              className={`h-1 flex-grow rounded-full transition-all ${
                s.id <= step ? 'bg-theme-yellow' : 'bg-white/10'
              }`}
            />
          ))}
        </div>
        ...
        <div className="mt-8">
          <button
            ...
            className="w-full bg-theme-yellow text-black font-black uppercase tracking-widest py-4 rounded-full hover:opacity-90 transition-opacity disabled:opacity-50"
          >
